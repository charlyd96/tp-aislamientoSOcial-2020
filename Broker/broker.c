...
void atenderSuscripcion(t_suscribe* suscriptor, int socket_cliente){
	switch(suscriptor->cola_suscribir){
		case NEW_POKEMON:{
			t_nodo* nodo_suscriptor = malloc(sizeof(t_nodo));
			pthread_t hilo_suscriptor;

			nodo_suscriptor->socket = socket_cliente;
			nodo_suscriptor->hilo = pthread_create(&hilo_suscriptor, NULL, (void*)mostrar, NULL);
			list_add(colaNewPokemon, nodo_suscriptor);

			//Revisar la forma de mostrar al proceso y a la cola
			log_info(logBroker, "El proceso %d se suscribió a la cola %d", suscriptor->tipo_suscripcion, suscriptor->cola_suscribir);
			break;
		}
		case APPEARED_POKEMON:{
			t_nodo* nodo_suscriptor = malloc(sizeof(t_nodo));
			...
			list_add(colaAppearedPokemon, nodo_suscriptor);
		...
		case CATCH_POKEMON:{
			...
			list_add(colaCatchPokemon, nodo_suscriptor);
		...
		default:{
			log_info(logBrokerInterno, "No se pudo realizar la suscripción.");
			break;
		}
	}
}
